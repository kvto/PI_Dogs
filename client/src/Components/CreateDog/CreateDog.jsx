...
const validate = function (input) {
    ...
    if (input.name){
        for(let i=0;i<input.name.length;i++){
            ...
            input.name[i]==='8' ||
            input.name[i]==='9'){
                error.name = "🚫 Unicamente LETRAS es valido 🚫"
            }
        }
    }
    if (Number(input.name)) error.name = "🚫 Unicamente letras es valido 🚫"
    if (parseInt(input.name.length) <= 1) error.name = "🚫 El nombre no es valido 🚫"
